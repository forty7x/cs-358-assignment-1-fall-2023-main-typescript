...
function doubleEachRecursive(list: List): List {
  if (list == null) return null;
   ...
   return cons(list.first*2, doubleEachRecursive(list.rest))
...
function reverse(list: List): List {
  if (list == null) return null;
    ...
    const reversed = reverse(list.rest);
    return snoc(list.first, reversed);
  }
...
function duplicateOnes(list: List): List {
  ...
  for (let curr: List = list; curr != null; curr = curr.rest) {
    if (curr.first == 1) {
      output = snoc(1, snoc(1, output));
    } else {
      output = snoc(curr.first, output);
    }
  }
  ...
  // if(list.first == 1){
  //   const doubledRest = duplicateOnes(list.rest);
  //   return cons(list.first, cons(list.first, doubledRest));
  // }
  // else {
  //   return cons(list.first, duplicateOnes(list.rest));
  // }
...
function countNodesRecursive(tree: LogicTree): number {
  ...
  switch (tree.tag) {
    ...
    case "not":
      return 1 + countNodesRecursive(tree.subtree);
...
function evaluate(tree: LogicTree): boolean {
  switch (tree.tag) {
    ...
    case "or":
      return evaluate(tree.leftSubtree) || evaluate(tree.rightSubtree);
...
function countOrs(tree: LogicTree): number {
  ...
  switch(tree.tag){
    case "or":
      return 1 + countOrs(tree.leftSubtree) + countOrs(tree.rightSubtree)
    case "and":
      return countOrs(tree.leftSubtree) + countOrs(tree.rightSubtree)
    case "not":
      return countOrs(tree.subtree)
    case "bool":
      return 0;
